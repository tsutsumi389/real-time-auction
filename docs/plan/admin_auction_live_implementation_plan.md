...
5. 参加入札者リストのリアルタイム更新
6. 主催者コントロールパネル
   - オークション開始（pending → active）
   - 次の価格開示（現在価格の更新）
   - オークション終了（active → ended）
7. システム管理者専用機能
...
[主催者] 確認ボタンをクリック
    ↓
[フロントエンド] POST /api/auctions/:id/start を送信
    ↓
[バックエンド]
    - オークション状態を 'active' に更新
    - 最初の商品の開始価格を current_price に設定
    - started_at タイムスタンプを記録
    ↓
...
**エラーハンドリング:**
- オークションが既に開始済み: 「このオークションは既に開始されています」
- 商品が登録されていない: 「商品が登録されていません。先に商品を登録してください」
...
[フロントエンド] POST /api/items/:id/open-price を送信
    ...
    ↓
[バックエンド]
    - 前回の価格で入札があったか確認（has_bid フラグ）
    - 入札なし → オークション終了処理（落札者確定）
    - 入札あり → 次の処理へ
    ↓
[バックエンド]
    - items.current_price を更新
    - price_history レコードを挿入（開示者、価格、タイムスタンプ）
...
- オークションが終了済み: 「このオークションは既に終了しています」
...
- 価格開示ボタン（確認モーダル付き）
- 商品終了ボタン（確認モーダル付き）
- 緊急停止ボタン（system_adminのみ、確認モーダル付き）
...
- `startAuction(itemId, startingPrice)`: オークション開始
...
3. 「開始」ボタンをクリック
4. 確認モーダルで確認
5. **期待結果**: オークション状態が 'active' に変更、現在価格が開始価格に設定
...
4. 「価格を開示する」ボタンをクリック
5. 確認モーダルで確認
6. **期待結果**: 現在価格が更新、価格開示履歴に新規エントリ追加
...
3. 「商品を終了」ボタンをクリック
4. 確認モーダルで確認
5. **期待結果**: 商品状態が 'ended' に変更、落札者が確定、ポイントが消費

#### シナリオ5: 緊急停止（system_adminのみ）
...
- 入札がない状態で価格を開示した場合 → 自動的に前の価格で終了
- 商品がない状態でオークションを開始しようとした場合 → エラー
