...
3. フロントエンドでクエリパラメータ作成
  ↓
4. バックエンドAPIにリクエスト送信
   - パラメータ: keyword, status, my_auctions, page, sort
...
**リクエスト例（未ログイン）**:
```
GET /api/auctions?page=1&limit=20&keyword=春季&status=active&sort=created_at_desc
```

**リクエスト例（ログイン時）**:
```
GET /api/auctions?page=1&limit=20&status=active&my_auctions=true
...
**Repository層（データアクセス）**:
- `FindPublicAuctionsWithFilters()`: 公開されたオークション一覧取得（商品数、サムネイル画像含む）
- `FindPublicAuctionsWithBidStatus()`: ログイン時の一覧取得（入札状況含む）
...
6. **オークションAPI Client（bidderAuctionApi.ts）**:
   - `getBidderAuctionList()`: 一覧取得
   - エラーハンドリング
   - ログイン状態に応じたヘッダー設定

7. **オークション状態管理（bidderAuctionStore.ts）**:
   - 一覧データの保持
   - フィルタ・検索条件の保持
   - ページネーション状態の保持
   - ログイン状態の確認
...
- [ ] Repository層に公開オークション一覧取得、入札状況取得メソッドを実装
- [ ] Service層に一覧取得ロジックを実装（認証状態に応じた分岐）
- [ ] Handler層に一覧取得エンドポイントを実装（任意認証）
- [ ] ルーティング設定（認証ミドルウェアを任意に設定）
- [ ] 手動テスト（curlまたはPostman、未ログインとログイン両方）

### Phase 2: フロントエンド基盤（2-3時間）
- [ ] bidderAuctionApi.tsにgetBidderAuctionListを実装
