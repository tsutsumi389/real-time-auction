      ...
      >
          ...
          <div class="sm:flex sm:items-start">
            <div
              :class="[
                ...
                admin?.status === 'active' ? 'bg-red-100' : 'bg-green-100',
              ]"
            >
              <span :class="[admin?.status === 'active' ? 'text-red-600' : 'text-green-600', 'text-xl']">
                {{ admin?.status === 'active' ? '⚠' : '✓' }}
              </span>
            </div>
            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
              <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">
                アカウント{{ admin?.status === 'active' ? '停止' : '復活' }}の確認
              </h3>
              <div class="mt-2">
                <p class="text-sm text-gray-500">
                  {{ admin?.email }} のアカウントを{{ admin?.status === 'active' ? '停止' : '復活'
                  }}してもよろしいですか？
                </p>
                <p v-if="admin?.status === 'active'" class="mt-2 text-sm text-red-600">
        ...
        <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
          <button
            ...
            :disabled="loading"
            :class="[
              admin?.status === 'active' ? 'bg-red-600 hover:bg-red-700' : 'bg-green-600 hover:bg-green-700',
