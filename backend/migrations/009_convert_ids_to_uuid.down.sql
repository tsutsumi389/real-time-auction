-- Rollback: Convert UUIDs back to BIGSERIAL
-- WARNING: This rollback will lose UUID values and regenerate sequential IDs
...
-- Step 2: Add temporary BIGSERIAL columns
ALTER TABLE auctions ADD COLUMN id_bigint BIGSERIAL;
ALTER TABLE items ADD COLUMN id_bigint BIGSERIAL;
ALTER TABLE items ADD COLUMN auction_id_bigint BIGINT;

-- Step 3: Create mapping tables to preserve relationships
CREATE TEMP TABLE auction_id_mapping AS 
SELECT id, id_bigint FROM auctions;

CREATE TEMP TABLE item_id_mapping AS 
SELECT id, id_bigint FROM items;

-- Step 4: Populate BIGINT foreign key columns in dependent tables
UPDATE items i SET auction_id_bigint = aim.id_bigint 
FROM auction_id_mapping aim WHERE i.auction_id = aim.id;

ALTER TABLE item_media ADD COLUMN item_id_bigint BIGINT;
UPDATE item_media im SET item_id_bigint = iim.id_bigint 
FROM item_id_mapping iim WHERE im.item_id = iim.id;

ALTER TABLE bids ADD COLUMN auction_id_bigint BIGINT;
UPDATE bids b SET auction_id_bigint = aim.id_bigint 
FROM auction_id_mapping aim WHERE b.auction_id = aim.id;

ALTER TABLE price_history ADD COLUMN auction_id_bigint BIGINT;
UPDATE price_history ph SET auction_id_bigint = aim.id_bigint 
FROM auction_id_mapping aim WHERE ph.auction_id = aim.id;

ALTER TABLE point_history ADD COLUMN related_auction_id_bigint BIGINT;
UPDATE point_history ph SET related_auction_id_bigint = aim.id_bigint 
FROM auction_id_mapping aim WHERE ph.related_auction_id = aim.id;
...
-- Step 6: Drop UUID primary key constraints and columns
...
-- Step 7: Rename BIGINT columns to id
ALTER TABLE auctions RENAME COLUMN id_bigint TO id;
ALTER TABLE items RENAME COLUMN id_bigint TO id;
ALTER TABLE items RENAME COLUMN auction_id_bigint TO auction_id;
ALTER TABLE item_media RENAME COLUMN item_id_bigint TO item_id;
ALTER TABLE bids RENAME COLUMN auction_id_bigint TO auction_id;
ALTER TABLE price_history RENAME COLUMN auction_id_bigint TO auction_id;
ALTER TABLE point_history RENAME COLUMN related_auction_id_bigint TO related_auction_id;

-- Step 8: Add primary key constraints back
